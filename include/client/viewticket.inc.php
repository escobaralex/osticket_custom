...
<div class="msg">Asunto: <?=Format::htmlchars($ticket->getSubject())?></div>
<div>
    <?if($errors['err']) {?>
...
<br>
<div align="left">
    <span class="Icon thread">Seguimiento del Ticket</span>
    <div id="ticketthread">
        <?
	    //get messages
...
<div>
    ...
    <div id="reply" style="padding:10px 0 20px 40px;">
        <?if($ticket->isClosed()) {?>
        <div class="msg">Ticket será reabierto...</div>
        <?}?>
        <form action="view.php?id=<?=$id?>#reply" name="reply" method="post" enctype="multipart/form-data">
            ...
            <div align="left">
                Ingrese Mensaje <font class="error">*&nbsp;<?=$errors['message']?></font><br/>
                <textarea name="message" id="message" cols="60" rows="7" wrap="soft"><?=$info['message']?></textarea>
            </div>
            <? if($cfg->allowOnlineAttachments()) {?>
            <div align="left">
                Adjunte Archivo<br><input type="file" name="attachment" id="attachment" size=30px value="<?=$info['attachment']?>" /> 
